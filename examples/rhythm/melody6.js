...
function triggerBass (bar, beat, time) {
  // create same scale at lower octave
  const root = key + (oct - 1)
  const scale = nn.createScale(root, nn.modes[mode])

  let note
  if (bar % 2 == 0) { // every even bar
    note = scale[0] // play root
  } else { // every odd bar
    note = scale[4] // play fifth
  }
  // only play on first beat
  if (beat === 0) synth.triggerAttackRelease(note, '1n', time)
}
...
function play (time) {
  ...
  // play the "tonal center" as a bass note (for context)
  triggerBass(bar, beat, time)
